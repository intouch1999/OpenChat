<template>
    ...
    <div class=" absolute top-20 buttom-0 overflow-visible  rounded-full px-4 py-2 bg-primary z-50">
      <span class=" bg-primary rounded-full px-4 py-2 text-primary-content font-bold text-xl">{{ displayText }}<span class="inline-block w-2 h-0.5 bg-black rounded-full blink-animation "></span></span>
    </div>
        </div>
    ...
    <div class="fixed bottom-0 inset-x-0 bg-primary w-full mx-auto rounded-t-3xl  ">
        <div class="flex justify-center flex-row w-full">
            <div class="m-2 flex flex-row align-middle items-end justify-center w-3/4">
    <textarea  v-model="Promt" @keydown="justEnter($event)"
    ...
    <button :disabled="loading || !Promt" @click="handlesendMess()"  class="ml-2 mb-1 justify-center rounded-full btn btn-md bg-secondary border-none">
